...
- `pet_id` 唯一且格式合法。
- `rarity` 必须为定义枚举。
- 如果 `obtainable_by_egg == true`，则开蛋池中必须存在与其匹配的概率档位（由开蛋池接口保证）。
...
- “开蛋池（Gacha Pool）”通常也需要维护接口，但本文件聚焦龟种定义（`PetDefinition`）与紧急开关；如需补充开蛋池接口，建议新开 `docs/api/gacha.md` 或在本文件新增独立章节。
