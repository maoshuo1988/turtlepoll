...
（新增）宠物管理
├─ 宠物配置（龟种/特性/稀有度/概率/折扣）
├─ 运营开关（上架/下架/灰度/禁用能力）
├─ 规则参数（取整/上限/倍率/每日次数）
└─ 审计与回滚（版本化配置、发布记录、影响评估）
```
...
7) **宠物管理（特性由运营侧维护）**
	- 目标：把“龟种能力/参数”从代码/常量里解耦出来，运营可配置、可灰度、可回滚，且所有改动可追溯。
	- 直接动机：宠物文档（见 `prompt/project/宠物.md`）里涉及大量倍率/封顶/取整/每日次数规则，若全写死在代码里，会导致：
	  - 线上调参成本高（每次发版）
	  - 争议不可追溯（为什么今天和昨天不一样）
	  - 体验难迭代（活动期需要临时“增强/削弱/禁用”某些能力）
	- 运营侧维护范围建议：
	  - 龟种基础信息：name、rarity、icon/skin、描述文案
	  - 抽取概率与折扣：开蛋价格、折扣公式、重复返还比例
	  - 能力开关：某能力是否启用（用于紧急止血）
	  - 规则参数：倍率、封顶、每日次数、取整规则（floor/round）
	  - 生效策略：全量/灰度（按用户id哈希）、定时生效（活动窗口）
	  - 风控参数：欠账上限、欠账锁龟、异常奖励上限
	- 不建议运营侧直接维护：
	  - 资金账务“核心流程”本身（扣款/入池/出池/结算顺序应保持稳定）
	  - 任何会破坏幂等性的实现细节（例如重复结算、跨天逻辑）

#### 7.1 P0 特性如何“运营侧可用”（详细设计）

> 目标：把 `prompt/project/宠物.md` 里 P0 的规则（每日结算 + 欠账/补贴/生息 + 每日切换限制）
> 设计成运营后台能维护的“结构化能力配置”，并保证：
>
> - **强校验**：不会出现负倍率/无限上限/互斥冲突等“配置把系统搞崩”的情况
> - **强审计**：能回答“某用户某天为什么拿到这笔钱/为什么不能切龟”
> - **可灰度**：新规则对少量用户生效，验证无误再全量
> - **可回滚**：一键回到上一版本
> - **不破坏账务主链路**：运营只能调参与开关，不能改变结算顺序与幂等策略

##### 7.1.1 统一抽象：能力（Ability）= 触发点（Trigger）+ 参数（Params）+ 约束（Limits）

P0 能力都发生在两个关键触发点：

1) **DailySettle（每日结算）**：用户当天首次打开/登录触发一次
2) **EquipSwitch（切换龟种）**：用户切换装备龟种时触发

对运营侧来说，我们只开放“参数层”，触发点与结算顺序由后端固定。

建议你们把能力 JSON 做成类似结构（示例，字段名可按你们习惯调整）：

- `abilities_json.daily_settle`（每日结算能力集合）
	- `login_bonus`：每日登录加成
	- `spark_multiplier`：火花倍率加成
	- `debt`：欠账能力（允许负数、欠账上限、锁龟规则）
	- `debt_subsidy`：欠款补贴（余额为负时）
	- `deposit_interest`：存款生息（余额为正时）

- `abilities_json.equip_switch`
	- `daily_switch_limit`：每日切换次数限制（全局规则，默认 1）
	- `debt_lock`：欠账未还清禁止切换（与 debt 能力一起配置/联动）

##### 7.1.2 P0-1：每日登录加成（login_bonus）

**业务口径（来自宠物需求）：**

- 登录结算时额外奖励固定龟币（部分龟种随等级加成）
- 影响：每日登录 & 签到

**运营可配置字段：**

- `enabled`：是否开启
- `base_amount`：基础奖励（整数，单位：龟币）
- `level_amount_step`：每级额外增加（可选，整数）
- `min_level / max_level`：等级范围（默认 1..10）
- `daily_cap_amount`：每日封顶（可选；没填则不额外封顶，仅用于防配置过大）

**计算约定（后端固定，运营不可改）：**

- `bonus = base_amount + (level-1) * level_amount_step`
- 取整策略：整数运算（天然取整）

**一致性/幂等约束：**

- 同一天（北京时间）只结算一次；重复请求不重复发放
- 必须能在“结算明细”里看见此项（见 7.1.7）

**运营后台页面建议：**

- 宠物配置页 → 能力 →「每日登录加成」卡片
- 支持“预览”：输入 level=1/10 直接显示对应奖励

##### 7.1.3 P0-2：火花倍率加成（spark_multiplier）
...
- `enabled`
- `multiplier_base`：基础倍率（例如 lava：1.3）
- `multiplier_per_level`：每级额外倍率（例如 lava：0.03）
- `extra_cap_amount`：封顶（仅“翻倍额外部分”的上限，宠物文档示例为 400/天）

**计算约定（后端固定）：**

- 先按系统规则算出 `spark_reward_raw`
- 得到倍率：`m = multiplier_base + multiplier_per_level * level`
- 额外增量：`extra = floor(spark_reward_raw * (m - 1))`
- 应用封顶：`extra = min(extra, extra_cap_amount)`
- 最终：`spark_reward = spark_reward_raw + extra`

**运营后台页面建议：**

- 提供“模拟器”：输入 spark_reward_raw 与等级，输出 extra 与最终值
- 提供“封顶提示”：当 extra 命中过封顶，UI 明确提示“已封顶”

##### 7.1.4 P0-3：欠账能力（debt）& 欠账锁龟（debt_lock）

**业务口径：**

- 余额可为负；欠账上限由龟种决定
- 欠款未还清禁止切换龟种（错误码 `DEBT_UNPAID`）

**运营可配置字段（debt）：**

- `enabled`
- `min_balance`：允许的最低余额（负数，例如 -300）
- `lock_switch_when_debt`：是否启用欠账锁龟（建议固定为 true，但仍然可配以便紧急降级）

**后端固定约束：**

- 所有扣款动作必须应用 `min_balance` 校验
- 如果 `balance < 0` 且 `lock_switch_when_debt=true`：切龟直接拒绝（`DEBT_UNPAID`）
...
##### 7.1.5 P0-4：欠款补贴（debt_subsidy）

**业务口径：**

- 每日结算时若余额为负，为欠款按比例补贴到余额（帮助还款）

**运营可配置字段：**

- `enabled`
- `rate_base`：基础补贴率（例如 0.20）
- `rate_per_level`：每级额外补贴率（例如 0.005）
- `daily_cap_amount`：每日补贴封顶（建议必须有，避免配置错误导致巨额补贴）

**计算约定（后端固定）：**

- 仅当 `balance < 0` 生效
- `rate = rate_base + rate_per_level * level`
- `subsidy_raw = floor(abs(balance) * rate)`
- `subsidy = min(subsidy_raw, daily_cap_amount)`
- `balance += subsidy`

**与存款生息互斥（强约束）：**

- 同一次每日结算中：若 `balance < 0`，只允许执行“欠款补贴”，不执行“存款生息”

##### 7.1.6 P0-5：存款生息（deposit_interest）

**业务口径：**

- 每日结算时若余额为正，按比例发放利息，有每日上限

**运营可配置字段：**

- `enabled`
- `rate`：利率（例如 space 0.03、龟壳 0.05；也可以拆 base+perLevel）
- `interest_base_cap_balance`：参与计息的余额上限（例如 1000）
- `daily_cap_amount`：每日利息上限（建议与 cap_balance 二选一或同时生效）

**计算约定（后端固定）：**

- 仅当 `balance > 0` 生效
- `basis = min(balance, interest_base_cap_balance)`
- `interest_raw = floor(basis * rate)`
- `interest = min(interest_raw, daily_cap_amount)`（若配置了）
- `balance += interest`

##### 7.1.7 P0-6：每日切换限制（daily_switch_limit）

**业务口径：**

- 每日仅允许切换龟种一次（与头像框/场景无关）

**运营可配置字段：**

- `enabled`（建议默认 true）
- `daily_limit`：每日允许切换次数（默认 1；紧急情况下可调大以止血客服压力）
- `timezone`：固定为北京时间（运营不可变，仅展示）

**后端固定约束：**

- 切龟前先校验：
	1) 欠账锁龟（若开启）
	2) 当天切换次数是否超限（超限返回 `EQUIP_DAILY_LIMIT`）

##### 7.1.8 运营侧必须具备的“可解释性”能力（强烈建议）

为了让客服/仲裁能解释用户纠纷，宠物系统建议补 2 个运营能力页面：

1) **用户结算明细（按天）**
	- 输入 userId + date（北京时间）
	- 展示：
	  - 基础签到奖励
	  - 火花奖励 raw / extra / cap 命中情况
	  - 登录加成
	  - 欠款补贴 or 存款生息（二选一）
	  - 最终余额变化
	  - 当天生效的宠物配置版本号（snapshotVersion）

2) **用户切龟审计**
	- 展示当日切龟次数、最后一次切龟时间
	- 若被拒绝：显示具体错误码与配置来源

##### 7.1.9 配置发布流程（避免“运营误操作”）
...
- Publish：选择生效方式：
	- 立即全量
	- 定时全量
	- 灰度（例如 1%/5%/20% 按用户id哈希）

并强制写 operate-log：

- 操作人、原因、diff 摘要、灰度策略、生效时间
...
2) 运营编辑上下文（迁移到 admin）
	- （建议新增）`POST /api/admin/predict/context_update`
...
### 7.3 Pet（宠物）运营接口（建议新增）

> 原则：宠物“特性/参数”可配置，但必须可审计、可回滚、可灰度；并且要能在后端生成一份“当前生效配置快照”用于纠纷解释。

1) 宠物配置列表（运营侧查询）
	- `GET /api/admin/pet/defs`
	- 支持筛选：rarity、enabled、能力类型（signin/interest/debt/chat/egg/minigame）

2) 单个宠物配置详情
	- `GET /api/admin/pet/defs/:petId`
	- 返回：基础信息 + abilities（结构化）+ 生效策略 + 最近发布记录

3) Draft 编辑与校验（不直接生效）
	- `POST /api/admin/pet/defs/:petId/draft`
	- 需要服务端校验：
	  - 数值范围（倍率、封顶、次数）
	  - 互斥规则（如欠账能力与切换限制）
	  - 整数取整策略合法（仅允许 floor/ceil/round 一类枚举）

4) 发布/回滚（版本化）
	- `POST /api/admin/pet/defs/publish`（body: petId + draftVersion + effectiveAt + rollout）
	- `POST /api/admin/pet/defs/rollback`（body: petId + targetVersion）
	- 强制写入 operate-log（含 diff 摘要）

5) 紧急开关（止血）
	- `POST /api/admin/pet/ability/disable`（body: abilityKey + reason + scope）
	- scope 建议支持：全量 / 灰度 / 指定用户

6) 生效配置快照（对外解释用）
	- `GET /api/admin/pet/effective_snapshot?userId=...&at=...`
	- 说明：用于客服/仲裁“解释当时规则”，避免线上改参后无法复盘。

### 7.4 Pet（数据模型建议，落地数据库）

> 这里是“运营侧可维护”的数据结构建议，方便对齐后端表结构/JSON schema；不限制具体实现。

- `pet_definitions`（龟种配置主表）
	- pet_id（如 basic/stone/...）
	- name、rarity、enabled
	- display（icon、cover、description）
	- abilities_json（结构化能力配置）
	- pricing_json（开蛋/龟粮折扣等）
	- rollout_json（灰度策略、effective_at）
	- version、created_at、updated_at

- `pet_definition_revisions`（版本历史）
	- pet_id、version、diff_summary、full_snapshot_json、operator_id、reason、created_at

- `user_pet_state`（用户状态，运行时数据）
	- user_id、equipped_pet_id
	- pet_levels_json（每只龟的等级/经验）
	- last_switch_at（每日切换限制）
	- debt_lock（欠账锁龟标记）

> 注意：`user_pet_state` 里的资金余额仍以“系统金币/龟币=金币”为准，宠物只影响计算参数，不应引入新的独立币种。
