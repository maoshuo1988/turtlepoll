...
- 市场查询与上下文：`/api/football/**`（当前挂在 `/api` 下默认要登录）
  ...
  - 同步入口（手动触发）：`POST /api/football/sync_worldcup`、`POST /api/football/sync_polymarket`
...
3) 人工结算 / 作废
	- `POST /api/admin/predict/market/settle`（指定 A/B/void）
	- `POST /api/admin/predict/market/void`
	- 必须：幂等 + 操作日志 + 二次确认。

4) 同步任务监控
	- `POST /api/admin/predict/sync_polymarket`
	- `GET /api/admin/predict/sync_status`
